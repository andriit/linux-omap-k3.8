...
/**
 * sr_schedule_periodic_calibration() - schedules periodic calibration
 * @sr:		SmartReflex used for calibration
 * @volt:	Nominal voltage, which needs to be calibrated
 *
 * Schedules delayed work for offline calibration.
 */
static int sr_schedule_periodic_calibration(struct omap_sr *sr,
					    unsigned long volt)
{
	struct sr_periodic_calib_data *calib_data;
					 ...
					 unsigned long volt)
{
	 ...
	 */
	if (!res)
		res = sr_schedule_periodic_calibration(sr, volt);

	return res;
}

/**
 * sr_periodic_calibration_cancel() - cancels periodic calibration
 * @sr:		SmartReflex used for calibration
 *
 * Cancels delayed calibration work.
 */
static void sr_periodic_calibration_cancel(struct omap_sr *sr)
{
	struct sr_periodic_calib_data *calib_data;
...
static int sr_stop_periodic_calibration(struct omap_sr *sr)
{
	...
	sr_periodic_calibration_cancel(sr);
...
static void sr_sw_calibration_work(struct work_struct *work)
{
	...
	/*
	 * Handle the case where we might have just been scheduled AND
	 * disable was called. Work is not rescheduled in this case.
	 */
	if (!mutex_trylock(&dvfs_lock)) {
		pr_err("%s: %s: stop SW calibration\n",
		       __func__, sr->name);
		return;
	}
