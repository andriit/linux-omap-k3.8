...
struct omap_abb_data {
	...
	u32 rbb_sel_mask;
	u32 fuse_vset_mask;
	u32 fuse_enable_mask;
	u32 ldovbb_mux_ctrl_mask;
	u32 ldovbb_vset_out_mask;
...
static const struct omap_abb_data __initdata omap5_abb_data = {
	...
	.fbb_sel_mask		= ABB_ACTIVE_FBB_SEL_MASK,
	.sr2_wtcnt_value_mask	= ABB_SR2_WTCNT_VALUE_MASK,
	.fuse_vset_mask		= ABB_FUSE_VSET_MASK,
	.fuse_enable_mask	= ABB_FUSE_ENABLE_MASK,
	.ldovbb_mux_ctrl_mask	= ABB_LDOVBBMPU_MUX_CTRL_MASK,
	.ldovbb_vset_out_mask	= ABB_LDOVBBMPU_VSET_OUT_MASK,
	.setup_offs		= 0,
	.control_offs		= 0x4,
	.settling_time		= 50,
	.clock_cycles		= 16,
...
static int omap_abb_init_oppsel_table(struct device *dev, struct omap_abb *abb)
{
	...
	for (i = 0; i < num_values; i++) {
		...
		if (efuse_val & data->fuse_enable_mask) {
			abb->opp_sel_table[i].opp_sel = OMAP_ABB_FAST_OPP;
			abb->opp_sel_table[i].efuse_vset =
				(efuse_val & data->fuse_vset_mask) >>
				 __ffs(data->fuse_vset_mask);
		} else {
			abb->opp_sel_table[i].opp_sel = OMAP_ABB_NOMINAL_OPP;
		}
	}
...
/**
 * omap_abb_program_ldovbb() - program LDOVBB register
 * @dev:	device pointer used to lookup device of_node
 ...
 * Returns 0 on success or error code otherwise
 */
static void omap_abb_program_ldovbb(const struct omap_abb *abb,
				    u32 opp_sel, u32 vset)
{
	const struct omap_abb_data *data = &abb->data;
	u32 val;
	...
	val &= ~(data->ldovbb_mux_ctrl_mask | data->ldovbb_vset_out_mask);

	switch (opp_sel) {
	case OMAP_ABB_SLOW_OPP:
	case OMAP_ABB_FAST_OPP:
		val |= data->ldovbb_mux_ctrl_mask;
		val |= vset << __ffs(data->ldovbb_vset_out_mask);
		break;
	}
...
static int omap_abb_set_opp(struct omap_abb *abb, int volt)
{
	...
	/* program LDOVBB */
	omap_abb_program_ldovbb(abb, new_opp_sel, abb_opp->efuse_vset);
...
static int omap_abb_info_dump(struct seq_file *sf, void *unused)
{
	...
	const struct omap_abb_opp_sel *opp_sel_table = abb->opp_sel_table;
	u32 abb_ctrl, abb_setup, ldovbb = 0;
	...
	seq_printf(sf, "Enabled\t->\t%d\n"
		   ...
		   "CONTROL_WKUP_LDOVBB_XXX_VOLTAGE_CTRL\t->\t0x%08x\n",
		   !!(abb_setup & abb->data.sr2en_mask),
		   abb->current_volt, abb->opp_sel,
		   !!(abb_setup & abb->data.fbb_sel_mask),
		   !!(abb_setup & abb->data.rbb_sel_mask),
		   abb_setup, abb_ctrl, ldovbb);
...
static int __init omap_abb_probe(struct platform_device *pdev)
{
	...
	/* optional resources */
	if (abb->data.fuse_vset_mask &&
	    abb->data.fuse_enable_mask &&
	    abb->data.ldovbb_mux_ctrl_mask &&
	    abb->data.ldovbb_vset_out_mask) {
		mem = platform_get_resource_byname(pdev, IORESOURCE_MEM,
						   "efuse");
		if (mem) {
			abb->efuse = devm_ioremap_nocache(&pdev->dev,
							  mem->start,
							  resource_size(mem));
			if (!abb->efuse) {
				ret = -ENOMEM;
		...
		mem = platform_get_resource_byname(pdev, IORESOURCE_MEM,
						   "ldovbb");
		if (mem) {
			abb->ldovbb = devm_ioremap_nocache(&pdev->dev,
							   mem->start,
							   resource_size(mem));
			if (!abb->ldovbb) {
				ret = -ENOMEM;
...
MODULE_DESCRIPTION("OMAP ABB regulator driver");
MODULE_LICENSE("GPL");
